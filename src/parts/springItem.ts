...
export class SpringItem extends MyObject3D {
  ...
  private _hokanSize: number = 0.01

  constructor(id:number, t: Texture) {
    super();
    ...
    this._mat = new ShaderMaterial({
      ...
      uniforms: {
        ...
        time: { value: 0 },
        col: { value: 0 },
        color: { value: Conf.instance.BASE_COLOR },
      },
    })
  ...
  protected _update():void {
    ...
    const allRadius = Math.min(sw, sh) * 0.25 * Util.map(Math.sin(this._c * 0.01), 0, 3, -1, 1)

    const ang = this._c * Util.map(this._id, 1, 1.5, 0, Conf.instance.ITEM_NUM - 1)
    const radianA = Util.radian(ang)
    const radianB = Util.radian(ang + 180 * Util.map(Math.cos(this._c * -0.023), 0, 1, -1, 1))
    ...
    this._hokan.forEach((hokan, i) => {
      hokan.scale.set(size * -1, size * 0.5, 1)
      ...
      if(p != undefined) {
        ...
        const dx = next.x - p.x
        const dy = next.y - p.y
        hokan.rotation.z = Util.radian(Util.degree(Math.atan2(dy, dx)) + 0)
      ...
      this._left.rotation.z = this._hokan[1].rotation.z
    })
  }
}